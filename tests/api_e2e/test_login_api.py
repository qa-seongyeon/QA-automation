...
def test_signup_and_login_response(request_context: APIRequestContext, account_data):
    ...
    assert create_body.get("responseCode") == 201
    assert create_body.get("message") == "User created!"
    ...
    assert login_body.get("responseCode") == 200
    assert login_body.get("message") == "User exists!"
    ...
    assert detail_body.get("responseCode") == 200
    user = detail_body.get("user", {})
    assert user.get("email") == account_data["email"]
    assert user.get("name") == account_data["name"]
    assert user.get("title") == account_data["title"]
    assert user.get("first_name") == account_data["firstname"]
    assert user.get("last_name") == account_data["lastname"]
    assert user.get("company") == account_data["company"]
    assert user.get("address1") == account_data["address1"]
    assert user.get("country") == account_data["country"]
    assert user.get("state") == account_data["state"]
    assert user.get("city") == account_data["city"]
    assert user.get("zipcode") == account_data["zipcode"]
...
def test_update_account_response(request_context: APIRequestContext, account_data):
    ...
    """
    assert login_api.create_account(request_context, **account_data).json().get("responseCode") == 201
    ...
    assert update_body.get("responseCode") == 200
    assert update_body.get("message") == "User updated!"
    ...
    assert old_login_body.get("responseCode") == 404
    assert old_login_body.get("message") == "User not found!"
    ...
    assert new_login_body.get("responseCode") == 200
    assert new_login_body.get("message") == "User exists!"
...
def test_delete_account_response(request_context: APIRequestContext, account_data):
    ...
    assert before_body.get("responseCode") == 200
    assert before_body.get("message") == "User exists!"

    # 2. DELETE /api/deleteAccount 로 계정 삭제
    delete_body = login_api.delete_account(
        request_context, account_data["email"], account_data["password"]
    ).json()
    assert delete_body.get("responseCode") == 200
    assert delete_body.get("message") == "Account deleted!"

    # 3. POST /api/verifyLogin 으로 삭제한 계정 재로그인 시도
    after_body = login_api.verify_login(
        request_context, account_data["email"], account_data["password"]
    ).json()
    assert after_body.get("responseCode") == 404
    assert after_body.get("message") == "User not found!"
